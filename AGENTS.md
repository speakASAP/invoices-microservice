...
Inbound internal routes use Auth RS256 only (`InternalAuthGuard` → `/auth/validate` + `@Roles`). Payments enrichment uses `Authorization: Bearer` with `PAYMENTS_SERVICE_TOKEN`.
