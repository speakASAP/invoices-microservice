...
describe('account invoice access', () => {
  ...
  it('lists invoices scoped by normalized stored customer email', async () => {
    const invoices = [{
      id: 'invoice-1',
      orderId: 'order-1',
...
describe('CustomerAuthGuard', () => {
  ...
  it('validates a bearer token through Auth and attaches normalized customer identity', async () => {
    ...
    expect(request.customerAuth).toEqual({ id: 'user-1', email: 'person@example.test', roles: ['customer'] });
  });
