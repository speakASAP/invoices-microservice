...
| Notifications | notifications-microservice | required | Invoice-ready delivery is an machine identity governed by the canonical service identity standard | credential delivery governed by the canonical service identity standard | Delivery returns false and is logged; the invoice remains issued and downloadable | src/invoices/notifications-client.service.ts |
...
| Payments | payments-microservice | required | Payment snapshot enrichment is an machine identity governed by the canonical service identity standard | credential delivery governed by the canonical service identity standard | Snapshot read returns null and is logged; the final tax invoice is still issued without enrichment | src/invoices/payments-client.service.ts |
...
| Orders | orders-microservice | required | Order snapshot reads use an machine identity governed by the canonical service identity standard; event consumption carries no caller authority | credential delivery governed by the canonical service identity standard | The snapshot read raises ORDER_READ_CONFIG_MISSING or a read failure and issuance is blocked rather than inventing order data | src/invoices/orders-client.service.ts, src/events/rabbitmq-orders.consumer.ts |
