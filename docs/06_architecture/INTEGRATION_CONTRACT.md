...
| Notifications | notifications-microservice | required | Invoice-ready delivery is an Auth-issued pair-specific RS256 bearer call with a target-scoped role | Pair credential delivered Vault -> ExternalSecret -> Kubernetes Secret -> secretKeyRef | Delivery returns false and is logged; the invoice remains issued and downloadable | src/invoices/notifications-client.service.ts |
...
| Payments | payments-microservice | required | Payment snapshot enrichment is an Auth-issued pair-specific RS256 bearer call with a target-scoped read role | Pair credential delivered Vault -> ExternalSecret -> Kubernetes Secret -> secretKeyRef | Snapshot read returns null and is logged; the final tax invoice is still issued without enrichment | src/invoices/payments-client.service.ts |
...
| Orders | orders-microservice | required | Order snapshot reads use an Auth-issued pair-specific RS256 bearer call with a target-scoped read role; event consumption carries no caller authority | Pair credential delivered Vault -> ExternalSecret -> Kubernetes Secret -> secretKeyRef | The snapshot read raises ORDER_READ_CONFIG_MISSING or a read failure and issuance is blocked rather than inventing order data | src/invoices/orders-client.service.ts, src/events/rabbitmq-orders.consumer.ts |
