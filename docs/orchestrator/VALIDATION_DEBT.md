...
Record known validation failures that are not caused by the current task, so agents can separate existing repository debt from real regressions.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| VD-001 | 2026-08-30 | `python3 ../intent-preservation-system/scripts/validate_adoption_profile.py --root . --phase planning` | No open validation debt recorded for the invoices-microservice IPS adoption bootstrap. | repo-wide | project owner | no | maintain clean adoption evidence | `docs/12_validation/VAL-TASK-001-bootstrap-service.md` |
| VD-002 | 2026-08-30 | runtime lifecycle-driven issuance smoke | Lifecycle-driven issuance cannot be exercised end to end while `ORDERS_EVENTS_CONSUMER_ENABLED` is `false` in the production ConfigMap; the consumer code exists and is bound to both routing keys. | runtime configuration | project owner | no | owner enables the Orders events consumer in the production ConfigMap | `k8s/configmap.yaml`, `src/events/rabbitmq-orders.consumer.ts` |
