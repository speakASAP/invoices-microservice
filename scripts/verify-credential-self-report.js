...
const vendorSource = path.join(root, 'src/common/vendor/credential-reporter.js');
...
check('reporter probes warehouse, not catalog or /health', () => {
  const text = fs.readFileSync(path.join(root, 'src/common/credential-self-reporter.ts'), 'utf8');
  assert(text.includes('/api/orders/customer/lifecycle'), 'does not probe the orders lifecycle read route');
...
check('principal string matches auth exactly', () => {
  ...
  assert(
    text.includes("'svc-invoices-microservice--orders-microservice@internal.alfares.cz'"),
...
check('nest-cli registers the vendor asset', () => {
  ...
  assert(
    assets.some((a) => (typeof a === 'string' ? a : a.include || '').includes('common/vendor')),
    'nest-cli.json does not register common/vendor/*.js — dist/ will lack the module',
  );
});
...
check('vendored module reached dist/ (run after build)', () => {
  ...
  const candidates = fs
    ...
    .map((e) => path.join(root, 'dist', e.name, 'common/credential-self-reporter.js'))
    .concat([path.join(root, 'dist/common/credential-self-reporter.js')]);
